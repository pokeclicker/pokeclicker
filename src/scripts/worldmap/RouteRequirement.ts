let moveToRoute = function (route: number, region: GameConstants.Region) {
    if (!isNaN(route) && !(route == Player.route)) {
        if (accessToRoute(route, region)) {
            Player.route = route;
        }
        else {
...
let accessToRoute = function (route: number, region: GameConstants.Region) {
