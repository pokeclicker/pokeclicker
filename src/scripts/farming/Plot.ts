class Plot implements Saveable {
    ...
    generateWanderPokemon(): any {
        ...
        if (Rand.chance(GameConstants.WANDER_RATE * App.game.farming.externalAuras[AuraType.Attract]())) {
            ...
            // Add to log book
            const translatedPokemon = PokemonHelper.displayName(wanderPokemon)();
            if (shiny) {
                App.game.logbook.newLog(LogBookTypes.SHINY, `A shiny ${translatedPokemon} has wandered onto the farm! ${App.game.party.alreadyCaughtPokemonByName(wanderPokemon, true) ? '(duplicate)' : ''}`);
            } else {
                App.game.logbook.newLog(LogBookTypes.WANDER, `A wild ${translatedPokemon} has wandered onto the farm!`);
            }
