...
pokemonList.forEach(p => {
    p.attack = Math.round(Math.max(Math.round(Math.max(1, Math.floor(Math.sqrt((2 * p.base.hitpoints)) * (2 * Math.round(Math.sqrt(p.base.attack * specialAttack) + Math.sqrt(p.base.speed)) * Math.sqrt(2 * Math.round(Math.sqrt(p.defense * p.base.specialDefense) + Math.sqrt(p.base.speed))))))) / 300));
    if (p.baby) {
