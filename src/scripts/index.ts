...
// Adapted from https://stackoverflow.com/questions/19305821/multiple-modals-overlay#answer-24914782
$(document).on('show.bs.modal', '.modal', function () {
    // setTimeout with 0 delay because the backdrop doesn't exist yet
    setTimeout(() => {
        const zIndex = Math.max(1040, Math.max(...$('.modal').filter(':visible').get().map(e => +e.style.zIndex)) + 10);
