...
export default class PokemonHelper {
    public static getEvolution = getEvolution;
    ...
    public static calcUniquePokemonsByRegion(region: Region) {
        return new Set(P.pokemonList.filter((p) => p.id > 0 && this.calcNativeRegion(p.name) === region).map((p) => Math.floor(p.id))).size;
    }
    ...
    public static getImage(pokemonId: number, shiny: boolean = undefined, gender: boolean = undefined, shadow: ShadowStatus = undefined): string {
    ...
    public static getPokeballImage(pokemonName: PokemonNameType): string {
        let src = '';
        if (App.game.party.alreadyCaughtPokemon(this.getPokemonByName(pokemonName).id)) {
            src = 'assets/images/pokeball/Pokeball-';
            if (App.game.party.alreadyCaughtPokemon(this.getPokemonByName(pokemonName).id, true)) {
                src += 'shiny-';
            }
            src += 'small.png';
        }
    ...
    public static displayName(englishName: string): Computed<string> {
        return App.translation.get(englishName, 'pokemon');
    }

    public static hasMegaEvolution(pokemonName: PokemonNameType): boolean {
        return !!P.pokemonMap[pokemonName].evolutions?.some((e) => e.restrictions.some((r) => r instanceof MegaEvolveRequirement));
    }

    public static hasUncaughtMegaEvolution(pokemonName: PokemonNameType): boolean {
    ...
    public static getMegaStones(pokemonName: PokemonNameType): MegaStoneItem[] {
    ...
    public static hasGigantamaxForm(pokemonName: PokemonNameType): boolean {
        return P.pokemonMap[`Gigantamax ${pokemonName}`].id > 0 || P.pokemonMap[`Eternamax ${pokemonName}`].id > 0;
    }

    public static hasUncaughtGigantamaxForm(pokemonName: PokemonNameType): boolean {
    ...
    public static isGigantamaxForm(pokemonName: PokemonNameType): boolean {
    ...
    public static incrementPokemonStatistics(pokemonId: number, statistic: PokemonStatisticsType, shiny: boolean, gender: number, shadow: ShadowStatus) {
